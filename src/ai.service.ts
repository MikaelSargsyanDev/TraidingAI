...
export class AiService {
  async getTweets(twitterPage) {
    const browser = await puppeteer.launch({ args: ['--no-sandbox'] });
    const page = await browser.newPage();

    await page.goto(`https://twitter.com/${twitterPage}`, {
  ...
  async getAIResponse(twitterPage) {
    const { tweets, username } = await this.getTweets(twitterPage);
